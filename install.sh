...
then
	echo "Installing i386 packages."
        ...
        bash -c "apt-get -y install $commonDependencies"
	echo "Installing URSIM-specific packages."
        dpkg -i $PWD/ursim-dependencies/*i386.deb
else
	echo "Installing amd64 packages."
        packages=`ls $PWD/ursim-dependencies/*amd64.deb`
	echo "Installing URSIM-specific packages."
        bash -c "apt-get -y install lib32gcc1 lib32stdc++6 libc6-i386 $commonDependencies" 
        dpkg -i $PWD/ursim-dependencies/*amd64.deb
fi
...
echo "Making URSIM executable."
chmod +x $PWD/URControl
